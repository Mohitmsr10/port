  ...
  const handleSubmit= (e) => {
    ...
    emailjs.send(
      ...
      import.meta.env.VITE_APP_EMAILJS_TEMPLATE_ID,
      {
        ...
        to_email: 'itzkartikeds3@gmail.com',
        message: form.message
      },
      import.meta.env.VITE_APP_EMAILJS_PUBLIC_KEY
    )   // It's an asynchronous operation, so we need to call .then on it
    .then (() => {
      setIsLoading(false);
      // Show success message
      showAlert({show: true, text:'Message sent successfully!', type: 'success'})
      
      // Stop running of fox after sometime of form submission
      setTimeout(() => {
        // Hide an alert
        hideAlert();

        setCurrentAnimation('idle')
        setForm({ name: '', email: '', message: '' });    // Clearing form 
      }, [3000])
    }) 
    .catch((error) => {
      setIsLoading(false);
    ...
    <section className='relative flex lg:flex-row flex-col max-container h-[100vh]'>
      ...
      <div className='flex-1 min-w-[50%] flex flex-col'>
        ...
        <form className='w-full flex flex-col gap-7 mt-14' onSubmit={handleSubmit}>
          ...
          <button type='submit' className='btn' disabled={isLoading} onFocus={handleFocus} onBlur={handleBlur}>   {/* If isLoading is True, button should be disabled */}
